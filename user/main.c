...
#define FIRST_ARG 1 // This and the next definition are just because I despise magic numbers.
#define LAST_ARG 2
...
rule_t *rule_table; // Will be used to point to the data we're writing to a rule table.
int fd;
// I wanted to use a fd and not a FILE because I know that when we write to a sysfs attribute,
// sysfs expects the entire buffer to be written during the first write,
// and a non-syscall write function can spread the writing as it sees fit.
...
*/
int main_deserialize_field(char **field, unsigned int member, char* keywords[], unsigned int values[], size_t len)
{
...
/*
    The next enum is for the cleanup function in main.c.
    Items represent states at which the program can be during exeution that should be cleaned up differently.
*/
enum stage{
    FIRST,
    FILE_OPENED,
...
{
    /*
    This function's logic is overly complex, but optimizing it will require some pondering and maybe serious changes in the code,
    So given time limitations the fact that this functions properly is enough for now.
    */
...
int main(int argc, char* argv[])
{
    ...
    if (argc == ARGC_FOR_NO_RULE_LOADING)
    {
        ...
        else if (strcmp(argv[FIRST_ARG], "clear_log") == SRTCMP_OF_EQ)
        {
            ...
            ERR_CHECK_INIT_MSG(write(fd, "A very functional and important text", MIN_STORE) == MAIN_SYSCALL_FAIL_RETURN, RESET_LOGS, MAIN_LOGS_ATTRIBUTE_WRITE_ERR_MSG) // Meme alert! We could have given any buffer to write and it would not have changed the functionality. See, we're being professional here.
            cleanup(RESET_LOGS);
        }
        else
        {
    ...
    else
    {
        ...
        ERR_CHECK_INIT_MSG(list_parse_lines(lst, fptr), LIST_INIT, MAIN_MALLOC_ERR_MSG)
