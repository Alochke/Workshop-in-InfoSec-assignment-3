...
/*
    ...
    Parameters:
    - list (klist*): A pointer to the list we're destroying, it will be log_list from list.c in practice.
...
{
    for (klist_iter_init(list, iter); klist_next(iter)->n_node != list->k_list->k_list; klist_del(iter->i_curr))
